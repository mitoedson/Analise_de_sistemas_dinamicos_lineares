...
As questões citadas tem como objetivo aplicar o **Método Direto de Lyapunov** para provar a estabilidade (geralmente assintótica, e frequentemente **global**) de sistemas dinâmicos **não lineares**, **sem** depender de linearização — trabalhando diretamente com a estrutura completa e exata do sistema.
...
Isso pode acontecer por diversos motivos estruturais:
- Cancelamento de variáveis (item c: $y$ desaparece)
- Formação de quadrados perfeitos que zeram numa curva (item b)
- Produtos que zeram em múltiplos eixos
- Qualquer outra estrutura algébrica que permita $\dot V=0$ fora da origem


O critério **geral e definitivo** é sempre: **você precisa simplificar $\dot V$ completamente e verificar, algebricamente, onde exatamente ela se anula** — se for só na origem, é estritamente negativa; se for em qualquer conjunto maior (reta, curva, eixo, etc.), é apenas semi-definida.
